...
Usage: .\test_admin_api.ps1 [OPTIONS]
...
Examples:
  .\test_admin_api.ps1                                    # Use defaults
  .\test_admin_api.ps1 -BaseUrl "http://localhost:3000"   # Different port
  .\test_admin_api.ps1 -RestaurantId "my-restaurant-123"  # Specific restaurant
  .\test_admin_api.ps1 -BaseUrl "https://api.restovoice.com"  # Production API
...
function Main {
    ...
    $result = Test-Endpoint "GET" "/calls/$RestaurantId?limit=5&offset=0" "" "Get Call Logs (Pagination)" 200
    $testResults += @{ Test = "Call Logs"; Passed = $result }
    
    # Test 7: Get Call Logs with different pagination
    $result = Test-Endpoint "GET" "/calls/$RestaurantId?limit=10&offset=5" "" "Get Call Logs (Different Page)" 200
    $testResults += @{ Test = "Call Logs (Page 2)"; Passed = $result }
    
    # Test 8: Get Booking Logs
    $result = Test-Endpoint "GET" "/bookings/$RestaurantId?limit=5&offset=0" "" "Get Booking Logs" 200
    $testResults += @{ Test = "Booking Logs"; Passed = $result }
    
    # Test 9: Get Booking Logs with Status Filter
    $result = Test-Endpoint "GET" "/bookings/$RestaurantId?status=CONFIRMED&limit=5&offset=0" "" "Get Booking Logs (Status Filter)" 200
    $testResults += @{ Test = "Booking Logs (Confirmed)"; Passed = $result }
    
    # Test 10: Get Booking Logs with Failed Status
    $result = Test-Endpoint "GET" "/bookings/$RestaurantId?status=FAILED&limit=5&offset=0" "" "Get Booking Logs (Failed Status)" 200
    ...
    $result = Test-Endpoint "GET" "/calls/$RestaurantId?limit=150" "" "Get Call Logs (Limit Too High)" 422
    $testResults += @{ Test = "Error - Limit Too High"; Passed = $result }
    
    # Test 14: Error Cases - Negative Offset
    $result = Test-Endpoint "GET" "/bookings/$RestaurantId?offset=-5" "" "Get Booking Logs (Negative Offset)" 422
